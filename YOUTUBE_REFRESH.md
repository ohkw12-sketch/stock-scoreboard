...
매일 한국시간 오전 8시, 이 저장소와 https://stock-scoreboard.pages.dev/ 만 사용한다.
오후 전체시장 갱신은 오전 발언·원문 확인 상태·기준일을 유지하며 가격만 갱신한다.
...
오전 실행이 9시를 넘겼거나 수동 복구인 경우 `youtube_refresh.py --force`를 사용한다.
