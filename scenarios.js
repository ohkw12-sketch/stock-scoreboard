...
(() => {
  ...
  function candidate(s, key, watching=false) {
    ...
    const badge=key==='up'?(watching?(c.early?'선행 관찰':'가격 대기'):c.volume2x?'2배 거래량 돌파':c.strongBreakout?'돌파·거래량 강신호':c.state):(s.heatObservation?'과열 관찰':c.state);
    return `<article class="candidate"><div class="candidate-head">${stockLink(s)}${pill(badge,c.ready?'green':c.early?'blue':'amber')}</div>
  ...
  function growthLeaders() {
    const c=data.scenarios.up, ids=c.watchTickers||[], early=c.earlyTickers||[];
    return `${section('선행 관찰 · 추세 회복', `<span class="muted">표시 ${num(early.length)}개 · 전체 ${num(c.earlyCount??early.length)}개</span>`)}
      <div class="card growth-leaders"><p class="market-note">기업 성장 기준과 추세·상대성과·거래량 유지 조건을 통과했습니다. 큰 매물대 접근까지 확인된 종목만 ‘진입 검토’로 구분합니다.</p>
      ${early.length?`<div class="table-scroll"><table><thead><tr><th>기업</th><th>가격 상태</th><th class="num">20일 시장 대비</th><th class="num">3일 거래량</th></tr></thead><tbody>${early.map(t=>{const s=data.stocks[t],p=s.price,u=s.scenarios.up;return `<tr><td>${stockLink(s)}</td><td>${pill(u.ready?'진입 검토':'선행 관찰',u.ready?'green':'blue')}</td><td class="num">${percent(p.rs20)}</td><td class="num">${mult(p.volumeRatio)}</td></tr>`;}).join('')}</tbody></table></div>`:empty('현재 선행 관찰 종목이 없습니다')}</div>
      ${section('기업 성장 우선후보 · 가격 대기', `<span class="muted">표시 ${num(ids.length)}개 · 전체 ${num(c.watchCount??ids.length)}개</span>`)}
      <div class="card growth-leaders"><p class="market-note">올해·내년 예상 영업이익과 최근 확정 분기 실적이 모두 강한 기업 중 진입 조건을 기다리는 종목입니다. 추세·상대성과·거래량이 회복된 기업에는 ‘선행 관찰’을 표시합니다. 과거 큰 매물대 접근까지 확인되면 위 진입 검토로 이동합니다. 예상 영업이익 증가액, 최근 확정 증가액 순으로 살펴보며 가격 조건은 순위에 반영하지 않습니다. 최대 20개, 동일 섹터 최대 3개입니다.</p>
      ${ids.length?`<div class="table-scroll"><table><thead><tr><th>기업</th><th class="num">내년 예상 OP 증가</th><th class="num">최근 분기 OP 증가</th><th class="num">최근 분기 마진</th><th>가격 상태</th></tr></thead><tbody>${ids.map((t,i)=>{const s=data.stocks[t],f=s.fundamentals,c=s.scenarios.up;return `<tr><td><span class="leader-rank">${i+1}</span>${stockLink(s)}</td><td class="num"><b>${eok(f.nextOPDelta)}</b><span class="cell-note">전년비 ${percent(f.nextOPGrowthPct)}</span></td><td class="num"><b>${eok(f.latestOPDelta)}</b><span class="cell-note">전년비 ${percent(f.latestOPGrowthPct)}</span></td><td class="num">${finite(f.latestMarginPct)?`${num(f.latestMarginPct,1)}%`:'미확인'}</td><td>${pill(c.early?'선행 관찰':'가격 대기',c.early?'blue':'amber')}<span class="cell-note">${esc(c.waiting.slice(0,2).join(' · ')||'다음 완료 거래일 확인')}</span></td></tr>`;}).join('')}</tbody></table></div>`:empty('현재 성장 우선후보가 없습니다','예상과 확정 실적을 함께 확인한 기업이 나올 때 표시합니다.')}</div>`;
  }
  ...
  function scenarios() {
    ...
    return heading('THREE SCENARIOS','하나의 정답 대신, 세 가지 준비','시장 판단과 무관하게 모든 시나리오를 볼 수 있습니다. 조건 충족은 해당 환경의 연구 조건이며 매수 확정이 아닙니다.')+
      ...
      <div class="callout">상승은 종가 20일선 이상·60일선의 97% 이상, 20일 시장 대비 −3%p 이상, 최근 거래량 0.8배 이상, 과거 큰 매물대 돌파 또는 저항 아래 8% 이내를 진입 검토로 봅니다. 종가 돌파와 거래량 1.5배·2배는 별도 강신호입니다. 진입 후보는 최대 5개 섹터에서 섹터당 3개를 표시합니다. <a href="#stocks">종목 탐색</a>에서 화면 밖 종목도 확인할 수 있습니다.</div>
  ...
  function audit() {
    ...
    return heading('AUDIT & METHODOLOGY','무엇으로 판단했고, 무엇이 비어 있나','선정 차이와 투자 성과를 구분합니다. 실제로 보유한 자료 범위와 현재 판단 기준을 공개합니다.',pill(data.meta.version,'blue'))+
      ...
      <section class="card"><h3>가격·선정 순서</h3><ul class="audit-list"><li>연간 매출 2,000억원, 20일 평균 거래대금 10억원, 60거래일 이력과 개선 근거를 확인합니다. 건설·바이오·제약 제외를 유지합니다.</li><li>반도체를 포함한 모든 연구 후보는 기업 실적 6~12개월 관점입니다. 단타 매수 신호를 만들지 않습니다.</li><li>보유한 과거 일봉 전체에서 거래대금이 뚜렷하게 몰린 가격대를 찾습니다. 20일 스윙고점과 이동평균선은 별도 참고값이며 매물대 기준이 아닙니다.</li><li>상승 진입 검토는 큰 매물대 저항선 아래 8% 이내 또는 종가 돌파를 확인합니다. 돌파와 거래량 1.5배 이상은 별도 강신호입니다. 박스권은 돌파 후 지지로 바뀐 매물대를 하단으로 잡고 상단·하단 반응이 각각 두 차례 이상 반복됐는지 확인합니다. 현재 종가가 하단 위 3% 이내이면서 박스 폭의 하위 25%여야 합니다.</li><li>최근 3일 거래량과 그 직전 14일은 겹치지 않게 비교합니다. 0.8배는 유지 조건, 돌파와 1.5배·2배는 강신호입니다. 일봉으로 나눈 거래대금은 실제 가격별 체결량이나 매물 소화 완료 판정이 아닙니다.</li><li>상승은 예상 OP 증가액, 최근 확정 OP 증가액 순으로 정렬하고 가격은 진입 상태로만 표시합니다. 박스권·하락은 기존 시나리오 정렬을 유지합니다. 합산 점수는 없습니다.</li><li>상승: 종가 ≥20일선·60일선의 97%, 시장 대비 −3%p 이내, 거래량 ≥0.8배, 큰 매물대 저항 아래 8% 이내 또는 돌파. 박스권: 반복 확인된 박스 하단 접근·하락 진정·고가권 종가. 하락: 상대 방어·60일선 유지·고점 대비 낙폭 15% 이내·이익 훼손 점검.</li><li>급등 후 종가 밀림은 제외. 과열 예외도 실적·전망 동반 개선, 거래량 2배, 확인된 돌파, 일중 상위 20% 마감이 함께 확인된 관찰 후보에만 적용합니다.</li></ul></section></div>
