(() => {
  ...
  function render(data) {
    return `<p>${esc(data.policy)} · ${esc(data.status?.status)} · 확인 ${esc(data.status?.checked_at || '미확인')} · 조회 ${esc(data.status?.from||'미확인')}~${esc(data.status?.to||'미확인')}</p>`+
      (data.rows?.length ? '<div class="tablewrap"><table><thead><tr><th>종목 / Guidance</th><th>대상기간 / 기준</th><th>가이던스 · 억원</th><th>컨센서스 · 억원</th><th>괴리율</th><th>발표 / 정정 / 출처</th></tr></thead><tbody>'+data.rows.map(r=>{
