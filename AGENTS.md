...
This explicit user approval supersedes the older Project 2 restriction against forward estimates. Project 2 value and fundamental prerequisites must use the current year's newest official guidance or external consensus values. For the same period and metric, use the newer publication; official guidance wins on the same date. If neither source provides a usable current-year sales and operating-profit pair, estimate current-year Q3/Q4 from verified current-year Q1/Q2 and the prior year's quarter pattern. Prior-year absolute sales, profit, margin and valuation must not enter the score or hard gates; prior-year quarterly values may supply seasonality ratios only. When both guidance and consensus are absent and seasonality is the only forecast basis, apply a five-point final-score confidence penalty. A current-year P/OP above its sector median receives a five-point penalty. Keep the strict rotation p11 reported-four-quarter rules unchanged.

## Split Project 2 display (2026-09-22)
The user approved replacing the single public Project 2 rank with two vertically stacked rankings. The upper market-interest growth rank uses company growth evidence 60% and the current sector rotation score 40%; P/OP, PER, sector discounts and all other valuation multiples are excluded. Keep overheated and late-stage sectors visible with an explicit entry-state warning. The lower absolute-value growth rank uses an absolute value score built from fixed-anchor P/OP and current-year first-half earnings quality, combined 50/50 with company growth evidence and the existing capped risk penalties. It does not use sector-relative valuation or sector market interest. Both tables retain the existing minimum sales, operating-margin, liquidity, evidence and recent-negative gates. Keep the lower absolute rows in the legacy `p2.rows` field for downstream compatibility.

## Sector trend and growth-validation update (2026-09-23)
This user approval supersedes the earlier p11 stock-selection and watch-candidate display rules. Project 1-1 is sector trend only: rank at most ten qualifying sectors with 5-day score 50%, 10-day score 30%, 20-day score 20%, plus recent ten-session top-20 persistence adjustment. Do not publish p11 stock picks or watch candidates. Use states 신규포착, 추세확인, 추세유지, 추세약화 and 이탈검토. Rotation has no financial hard gate.

Project 2 market-interest growth uses company growth evidence 80% and sector trend 20%. Absolute-value growth requires absolute value score at least 60 and P/OP at most 15, excluding finance and holding companies. Consensus-only evidence costs five points; a seasonality-only current-year estimate costs ten points. Forward growthRate must compare the same annual periods; Q2/Q1 flow is shown separately and never substituted for forward growth.

Growth evidence follows 예상실적·공시근거 → 실적·산업 선행검증 → 수급·주가 확인. Give explicit components to current-year Q1-to-Q2 operating-margin acceleration, official monthly industry indicators linked by verified product exposure, capacity changes, and sustained foreign/institution net buying. Capacity or market action cannot create a candidate alone. Missing optional industry, capacity, or investor-flow data is neutral rather than a penalty. Article, disclosure, flow, volume and breakout observations tied to one underlying business event form one capped expectation factor, not additive duplicate evidence.

Combined recommendations are the union of the two Project 2 top lists intersected with a trend-confirmed sector and anti-chase checks. Eligible sector states are 추세확인 or 추세유지, stages ② or ③, risk below 70; company price requires five-day return at most 10%, 20-day moving-average distance from -3% to +7%, and a positive 20-day moving-average slope. An empty intersection stays empty.
