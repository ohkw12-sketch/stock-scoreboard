...
`forecast_report_engine.py`는 예상실적 원자료 수집기다. `forecast_integration.py`가 올해·내년 연간값이 모두 있고 불일치 검토가 아닌 종목만 기존 성장근거 계산에 연결한다. 같은 기간의 유효 컨센서스를 우선하며, 컨센서스에 매출·영업이익 항목이 없을 때만 연결 연간 회사 가이던스로 보충한다. 확정실적 기반 가치 원점수와 KIS 추정치 상향 이력은 바꾸지 않는다.
...
- 신선도: 보고서일로부터 60일을 초과한 값은 이력에는 남지만 현재 컨센서스에서 제외한다.
...
- `forecast_consensus.json`: 최근 60일 자료 중 같은 기간 최신 보고서와 30일 이내인 종목·기간별 중앙값
...
보충자료도 증권사별 최신값 선택과 60일 신선도 규칙을 동일하게 적용한다.
