...
`forecast_report_engine.py`는 예상실적 원자료 수집기다. `forecast_integration.py`가 올해·내년 연간값이 모두 있고 불일치 검토가 아닌 종목만 기존 성장근거 계산에 연결한다. 같은 종목·기간·항목에서 컨센서스 자료 중 최신 집계값과 회사의 연결 연간 가이던스를 날짜로 비교해 더 최신인 값을 쓴다. 날짜가 같으면 회사가 직접 발표한 가이던스를 쓴다. 날짜와 증권사 수는 표시하되 별도 점수·가중치·감점을 주지 않는다. 확정실적 기반 가치 원점수와 KIS 추정치 상향 이력은 바꾸지 않는다.
...
- 최신값: 매년 1월 1일부터 모은 자료에서 같은 증권사의 최신 보고서를 남기고, 같은 종목·기간은 가장 최근 컨센서스 집계값을 만든다. 60일·90일 같은 일괄 만료 기준으로 제외하지 않는다.
...
- 현재 컨센서스: 올해와 내년 예상만 포함하고, 끝난 분기와 내후년 이후 수치는 원자료에만 남긴다. 같은 기간의 최신 보고서보다 30일 넘게 오래된 값은 현재 컨센서스에서 제외한다. 증권사 3곳 이상일 때 중앙값에서 3배 넘게 벗어난 값은 원자료에 남기고 별도 검토 목록으로 보낸다.
...
- `forecast_consensus.json`: 연초 이후 자료에서 같은 기간의 가장 최근 보고서와 30일 이내인 증권사별 최신값을 모은 종목·기간별 중앙값. 이 값이 해당 기간의 최신 컨센서스 집계값이다.
...
보충자료도 증권사별 최신값 선택 규칙을 동일하게 적용한다.
