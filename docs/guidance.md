...
- `guidance.js`: 가치성장 탭 안의 접이식 Guidance 표. 가이던스와 외부 컨센서스·기준일을 함께 표시.
- `forecast_integration.py`: 같은 종목·기간·항목에서 최신 컨센서스와 연결 연간 가이던스 중 발표일이 더 최신인 값을 사용. 같은 날이면 가이던스를 우선. 날짜·증권사 수 점수·가중치 없음.
...
가이던스가 없다고 감점하지 않는다. 같은 종목·기간·항목에서 최신 컨센서스 집계일과 연결 연간 가이던스 발표일을 비교해 더 최신인 값을 쓴다. 날짜가 같으면 회사가 직접 발표한 가이던스를 쓴다. 한쪽에 해당 항목이 없으면 다른 쪽의 값을 쓴다. 자료 날짜와 증권사 수는 표시만 하며 별도 점수·가중치·감점을 주지 않는다. 반기·분기·별도·자회사 가이던스를 연간 연결 수치로 바꾸지 않는다. 확정실적 가치 원점수에는 미래 전망을 넣지 않는다.
