...
- `forecast_integration.py`: 60일 이내 유효 컨센서스를 성장근거에 우선 적용하고, 같은 기간·항목 컨센서스가 없을 때만 연결 연간 가이던스로 보충.
...
가이던스가 없다고 감점하지 않는다. 60일 이내 유효 컨센서스를 같은 기간 성장근거의 매출·영업이익에 우선하고, 해당 항목이 없을 때만 연결 연간 가이던스로 보충한다. 반기·분기·별도·자회사 가이던스를 연간 연결 수치로 바꾸지 않는다. 확정실적 가치 원점수에는 미래 전망을 넣지 않는다.
