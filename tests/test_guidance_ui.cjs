...
test('escaping and independent guidance fields',()=>{
 ...
 assert.match(html,/&lt;script&gt;/);assert.match(html,/100~120/);assert.match(html,/\+10.0%/);assert.match(html,/정정/);assert.match(html,/가이던스 · 억원/);assert.doesNotMatch(html,/가이던스 우선/);assert.ok(!html.includes('javascript:'));
});
